 ...
 * Date: 02/07/16
 *
 * Creates timer and method to load dictionary to dictionary structures
 */ 
#include <iostream>
#include <sstream>
#include <vector>
...
void Utils::load_dict(DictionaryBST& dict, istream& words, unsigned int num_words)
{
  ...
  for(; j < num_words; j++)
    ...
    // prints out warning if didn't get num_words words in dictionary
    if (realSize < num_words) cout << "warning: dict size too small: size is " 
                                   << realSize << "\n"; 
}
...
void Utils::load_dict(DictionaryHashtable& dict, istream& words, unsigned int num_words)
{
  ...
  for(; j < num_words; j++)
  {
    ...
    // prints out warning if didn't get num_words word in dictionary
    if (realSize < num_words) cout << "warning: dict size too small: size is " 
                                   << realSize << "\n";      
}
